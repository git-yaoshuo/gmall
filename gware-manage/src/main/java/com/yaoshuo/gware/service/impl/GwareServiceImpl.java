package com.yaoshuo.gware.service.impl;


import com.alibaba.fastjson.JSON;

import com.yaoshuo.gware.util.HttpclientUtil;
...
import com.yaoshuo.gware.bean.WareOrderTaskDetail;
import com.yaoshuo.gware.bean.WareSku;
import com.yaoshuo.gware.enums.TaskStatus;
import com.yaoshuo.gware.mapper.WareInfoMapper;
...
public class GwareServiceImpl implements GwareService {
     ...
     */
     public Integer  getStockBySkuId(String skuid){
         return wareSkuMapper.selectStockBySkuid(skuid);
     }
    ...
    public List<WareInfo> getWareInfoBySkuid(String skuid){
        return wareInfoMapper.selectWareInfoBySkuid(skuid);
    }
    ...
    public List<WareInfo> getWareInfoList(){
        return wareInfoMapper.selectAll();
    }
     ...
     */
    @Override
    public void sendSkuDeductStatus(String orderId) {
        ...
        try {
            connection = activeMQUtil.getConn();
            connection.start();

            session = connection.createSession(true, Session.SESSION_TRANSACTED);

            Queue queue = session.createQueue("SKU_DEDUCT_QUEUE");
            producer = session.createProducer(queue);

            ActiveMQMapMessage activeMQMapMessage = new ActiveMQMapMessage();

            WareOrderTask wareOrderTaskQuery = new WareOrderTask();
            wareOrderTaskQuery.setOrderId(orderId);
            WareOrderTask wareOrderTask = wareOrderTaskMapper.selectOne(wareOrderTaskQuery);
            TaskStatus taskStatus = wareOrderTask.getTaskStatus();

            activeMQMapMessage.setString("orderId", orderId);
            activeMQMapMessage.setString("taskStatus", taskStatus + "");
    ...
    public List<WareSku> getWareSkuList(){
        return wareSkuMapper.selectWareSkuAll();
    }
